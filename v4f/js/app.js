    ...
    if (opciones.length > 0) {
        ...
        opcionBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // GUARDAR PROGRESO (tanto aciertos como fallos) si usuario logeado
                if (currentUser) {
                ...
                resultadoContainer.innerHTML = `
                    ...
                    ">
                        ...
                        ${currentUser ? '<p style="margin-top: 0.5rem; color: #4CAF50;">💾 Progreso guardado</p>' : ''}
                        ${!currentUser ? '<p style="margin-top: 0.5rem; color: #ff9800;">⚠️ Inicia sesión para guardar tu progreso</p>' : ''}
                    </div>
                `;
            });
        });
...
function setupAuthForms() {
    ...
    document.getElementById('login-form').addEventListener('submit', async function(e) {
        ...
        if (result.success) {
            ...
            setTimeout(() => {
                closeLoginModal();
                // No recargamos, el listener de Firebase actualizará la UI
            }, 1000);
        } else {
            showAuthError(result.error || 'Email o contraseña incorrectos');
        }
    });
    ...
    document.getElementById('register-form').addEventListener('submit', async function(e) {
        ...
        if (result.success) {
            ...
            showAuthError(result.error || 'Este email ya está registrado');
        }
    });
}
...
async function handleLogout() {
    if (confirm('¿Seguro que quieres cerrar sesión?')) {
        await logout();
        // No recargamos, el listener de Firebase actualizará la UI automáticamente
    }
}
...
function markExerciseAttempt(exerciseId, isCorrect) {
    ...
    if (!currentUser.progress.totalAttempts) currentUser.progress.totalAttempts = 0;
    if (!currentUser.progress.correctAttempts) currentUser.progress.correctAttempts = 0;
    if (!currentUser.progress.completedExercises) currentUser.progress.completedExercises = [];

    // Incrementar intentos totales
    currentUser.progress.totalAttempts++;

    if (isCorrect) {
        // Incrementar aciertos
        currentUser.progress.correctAttempts++;
        
        // Añadir a completados si no está ya
        if (!currentUser.progress.completedExercises.includes(exerciseId)) {
            currentUser.progress.completedExercises.push(exerciseId);
            // Solo sumar tiempo en el primer acierto del ejercicio
    ...
    const accuracy = currentUser.progress.totalAttempts > 0 
        ? Math.round((currentUser.progress.correctAttempts / currentUser.progress.totalAttempts) * 100) 
        : 0;
    
    console.log(`📊 Intento registrado: ${isCorrect ? '✅' : '❌'}`);
    console.log(`   Total intentos: ${currentUser.progress.totalAttempts}`);
    console.log(`   Aciertos: ${currentUser.progress.correctAttempts}`);
...
function loadUserStats() {
    ...
    // Calcular precisión REAL (aciertos / intentos totales)
    const accuracy = totalAttempts > 0 
        ? Math.round((correctAttempts / totalAttempts) * 100) 
        : 0;
    document.getElementById('stat-accuracy').textContent = accuracy + '%';
    
    // Mostrar info de intentos
    const attemptsInfo = document.getElementById('stat-attempts-info');
    if (attemptsInfo) {
        attemptsInfo.textContent = `${correctAttempts}/${totalAttempts} intentos`;
    }

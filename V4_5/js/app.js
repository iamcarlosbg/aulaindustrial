...
function loadUserStats() {
    ...
    // Calcular precisión (simulada por ahora)
    const accuracy = completed.length > 0 ? Math.floor(75 + Math.random() * 20) : 0;
    document.getElementById('stat-accuracy').textContent = accuracy + '%';
    
    // Tiempo total (simulado)
    const totalTime = Math.floor(completed.length * 0.3);
    document.getElementById('stat-time').textContent = totalTime + 'h';
    
    // Racha (simulada)
    const streak = Math.min(completed.length, 15);

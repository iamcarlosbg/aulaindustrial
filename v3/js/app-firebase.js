...
class IndustrainApp {
            ...
            ">
                ...
                <form id="login-form" style="display: block;">
                    <div style="margin-bottom: 1.5rem;">
                        ...
                        <input type="text" id="login-email" required style="
                            width: 100%;
                            padding: 0.75rem;
                ...
                <form id="register-form" style="display: none;">
                    ...
                    <div style="margin-bottom: 1rem;">
                        <label style="display: block; margin-bottom: 0.5rem; color: var(--text-secondary); font-size: 0.9rem;">Usuario *</label>
                        <input type="text" id="register-email" required style="
                            width: 100%;
                            padding: 0.75rem;
                            background: var(--tertiary-bg);
                            border: 1px solid var(--border-color);
    ...
    async handleLogin() {
        ...
        try {
            const result = await window.firebaseAuthService.loginWithEmail(username, password);
    ...
    async handleRegister() {
        ...
        if (!email || !password) {
            ...
            messageDiv.style.background = 'rgba(239, 68, 68, 0.1)';
            messageDiv.style.color = 'var(--accent-danger)';
            messageDiv.textContent = '❌ Email y contraseña son obligatorios';
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                setTimeout(async () => {
                    const loginResult = await window.firebaseAuthService.loginWithEmail(email, password);
                    if (loginResult.success) {
                        document.getElementById('auth-modal')?.remove();
                        document.getElementById('login-btn')?.remove();
                        this.showUserInfo(loginResult.user);
                        await this.loadServerProgress();
                    }
                }, 1500);
            } else {
    ...
    async shareProgress(platform) {
        const progress = await window.firebaseAuthService.getMyProgress();
        if (!progress) {
            alert('❌ Error obteniendo progreso');
            return;
        }
        
        const stats = {
            completedExercises: progress.totalCompleted || 0,
            totalExercises: 600,
            successRate: progress.userData.totalExercises > 0
                ? Math.round((progress.userData.completedExercises / progress.userData.totalExercises) * 100)
                : 0
        };
        
        switch(platform) {
            case 'linkedin':
                window.socialShareService.shareOnLinkedIn(stats);
                break;
            case 'facebook':
                window.socialShareService.shareOnFacebook(stats);
                break;
            case 'twitter':
                window.socialShareService.shareOnTwitter(stats);
                break;
            case 'whatsapp':
                window.socialShareService.shareOnWhatsApp(stats);
                break;
            case 'copy':
                const result = await window.socialShareService.copyLink(stats);
                if (result.success) {
    ...
    /**
     * Toggle suscripción newsletter
     */
    async toggleNewsletter(subscribe) {
        const result = await window.firebaseAuthService.subscribeToNewsletter(subscribe);
        ...
        if (result.success) {
            ...
            document.getElementById('newsletter-toggle-profile').checked = !subscribe;
        }
    }
